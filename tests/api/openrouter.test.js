...
import { RateLimitError } from '../../src/api/rate-limiter.js';
...
describe('callOpenRouter', () => {
  ...
  it('should throw RateLimitError on 429 response', async () => {
    ...
    const res = createMockResponse(429, { 'retry-after': '60' });
    mockReq._callback(res);
    res.emit('data', JSON.stringify({ error: { message: 'Rate limited' } }));
    res.emit('end');

    await expect(promise).rejects.toThrow(RateLimitError);
    await expect(promise).rejects.toThrow('OpenRouter rate limit exceeded');
  });
  ...
  it('should include existing content in prompt', async () => {
    const promise = callOpenRouter('Update function', '', 'output.js', null, [], 'existing code');
    ...
    const requestData = JSON.parse(mockReq.write.mock.calls[0][0]);
    expect(requestData.messages[1].content).toContain('Existing file content:');
    expect(requestData.messages[1].content).toContain('existing code');
  });
  ...
  it('should handle context file read errors gracefully', async () => {
    ...
    vi.mocked(cleanCodeResponse).mockReturnValue('code');
    const promise = callOpenRouter('test', '', 'output.js', null, ['/missing.js']);
