...
import { readFileContent, writeFileContent, getLanguageFromFile, expandContextPaths } from '../../src/utils/file-utils.js';
...
describe('expandContextPaths', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('should return empty array for empty input', async () => {
    expect(await expandContextPaths([])).toEqual([]);
    expect(await expandContextPaths(null)).toEqual([]);
    expect(await expandContextPaths(undefined)).toEqual([]);
  });

  it('should return file paths unchanged', async () => {
    vi.mocked(fs.stat).mockResolvedValue({ isDirectory: () => false });

    const result = await expandContextPaths(['/path/to/file.js']);

    expect(result).toEqual(['/path/to/file.js']);
  });

  it('should expand directory to all files recursively', async () => {
    ...
    const result = await expandContextPaths(['/docs']);

    expect(result).toContain('/docs/file1.md');
    expect(result).toContain('/docs/subdir/file2.md');
    expect(result).toHaveLength(2);
  });

  it('should handle mixed files and directories', async () => {
    vi.mocked(fs.stat)
      .mockResolvedValueOnce({ isDirectory: () => false })  // file.js
    ...
    const result = await expandContextPaths(['/path/file.js', '/docs']);

    expect(result).toContain('/path/file.js');
    expect(result).toContain('/docs/readme.md');
  });

  it('should include non-existent paths for later error handling', async () => {
    const error = new Error('ENOENT');
    error.code = 'ENOENT';
    vi.mocked(fs.stat).mockRejectedValue(error);

    const result = await expandContextPaths(['/nonexistent/path']);

    expect(result).toContain('/nonexistent/path');
  });

  it('should expand home path in directories', async () => {
    const originalHome = process.env.HOME;
    process.env.HOME = '/home/testuser';
    ...
    expect(result).toContain('/home/testuser/docs/file.md');
